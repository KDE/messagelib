...
{
    QStringList lst;
    const KConfigGroup grp(KSharedConfig::openConfig(), u"General"_s);
    const int numberFilter = grp.readEntry("NumberFilter").toInt();
    lst.reserve(numberFilter);
    for (int i = 0; i < numberFilter; ++i) {
        const KConfigGroup newGroup(KSharedConfig::openConfig(), u"Filter_%1"_s.arg(i));
...
QList<FilterSavedManager::FilterInfo> FilterSavedManager::filterInfos() const
{
    ...
    for (int i = 0; i < numberFilter; ++i) {
        ...
        const FilterSavedManager::FilterInfo info{
            .filterName = newGroup.readEntry(u"name"_s),
            .identifier = newGroup.readEntry(u"identifier"_s),
            .iconName = newGroup.readEntry(u"iconName"_s),
        };
        lst << info;
    }
    return lst;
...
Filter *FilterSavedManager::loadFilter(const QString &identifier)
{
    ...
    for (const QString &group : list) {
        const KConfigGroup newGroup(KSharedConfig::openConfig(), group);
